...
import minicraft.item.CountInventoryStrategy;
...
public class Player extends Mob implements ItemHolder, ClientTickable {
		...
		playerInventory = new Inventory() {
			...
			public void add(int idx, Item item) {
				if (Game.isMode("Creative")) {
					if (executeStrategy(CountInventoryStrategy.class, item) > 0) return;

					item = item.clone();
					if (item instanceof StackableItem) ((StackableItem) item).count = 1;
				}
				super.executeStrategy(AddInventoryStrategy.class, idx, item);
			}
			...
			public Item remove(int idx) {
				if (Game.isMode("Creative")) {
					...
					if (executeStrategy(CountInventoryStrategy.class, currentItem) == 1) {
						super.executeStrategy(RemoveInventoryStrategy.class, idx);
						super.executeStrategy(AddInventoryStrategy.class,0, currentItem);
						return currentItem.clone();
					}
				}
				//return super.executeStrategy(RemoveInventoryStrategy.class, idx);
								return activeItem;
			}
		};
	...
	public void tick() {
        ...
        if (Game.getDisplay() == null && !Bed.inBed(this)) {
            ...
            if ((input.getKey("menu").clicked || input.getKey("craft").clicked) && activeItem != null) {
                playerInventory.executeStrategy(AddInventoryStrategy.class, 0, activeItem);
                activeItem = null;
            }
	...
	public void resolveHeldItem() {
		...
		if (!(activeItem instanceof PowerGloveItem)) { 
			...
			if (previousItem != null && !Game.isMode("Creative")) 
				...
				playerInventory.executeStrategy(AddInventoryStrategy.class, 0, previousItem); 
		} else {
    ...
    protected void attack() {
    	...
    	if (activeItem != null) {
    		...
    		if (activeItem instanceof ToolItem && stamina - 1 >= 0) {
    			...
    			if (tool.type == ToolType.Bow && tool.durability > 0 && playerInventory.executeStrategy(CountInventoryStrategy.class, Items.arrowItem) > 0) {

    				if (!Game.isMode("Creative")) playerInventory.executeStrategy(RemoveInventoryStrategy.class, Items.arrowItem);
    ...
    public void pickupItem(ItemEntity itemEntity) {
    	...
    	if (itemEntity.item instanceof StackableItem && ((StackableItem) itemEntity.item).stacksWith(activeItem)) {
    		...
    		playerInventory.executeStrategy(AddInventoryStrategy.class, itemEntity.item); // Add item to inventory
    	}
    }
    ...
    public void die() {
    	...
    	if (activeItem != null) deathChest.getInventory().executeStrategy(AddInventoryStrategy.class, activeItem);
    	if (currentArmor != null) deathChest.getInventory().executeStrategy(AddInventoryStrategy.class,currentArmor);

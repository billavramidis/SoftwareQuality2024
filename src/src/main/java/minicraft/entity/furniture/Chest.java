...
import minicraft.item.TryAddInventoryStrategy;
	...
	public void fillInventoryRandom(String lootTable, int depth) {
		try {
			...
			for (String line : lines) {
				...
				if (!line.startsWith(":")) {
					chestInventory.executeStrategy(TryAddInventoryStrategy.class, Integer.parseInt(data[0]), Items.get(data[1]), data.length < 3 ? 1 : Integer.parseInt(data[2]));
					...
					for (String item : fallbacks) {
						chestInventory.executeStrategy(AddInventoryStrategy.class, Items.get(item.split(",")[0]), Integer.parseInt(item.split(",")[1]));
					}
				}
			}

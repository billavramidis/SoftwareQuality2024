...
public class CraftingDisplay extends Display {
	...
	private int getCurItemCount() {
		return player.getInventory().executeStrategy(CountInventoryStrategy.class,recipes[recipeMenu.getSelection()].getProduct());
	}
	...
	private ItemListing[] getCurItemCosts() {
		...
		for (String itemName : costMap.keySet()) {
			...
			costList.add(new ItemListing(cost, player.getInventory().executeStrategy(CountInventoryStrategy.class, cost) + "/" + costMap.get(itemName)));
		}

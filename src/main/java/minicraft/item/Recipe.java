...
public class Recipe {
	...
	public boolean craft(Player player) {
		...
		if (!Game.isMode("Creative")) {
			...
			for (String cost : costs.keySet().toArray(new String[0])) {
				if (!cost.contains("GRIMOIRE")) player.getInventory().executeStrategy(RemoveInventoryStrategy.class, Items.get(cost), costs.get(cost));
			}
		}

		// add the crafted items.
		for (int i = 0; i < amount; i++) {
			player.getInventory().executeStrategy(AddInventoryStrategy.class, getProduct());
		}
	...
	private boolean getCanCraft(Player player) {
		...
		for (String cost : costs.keySet().toArray(new String[0])) { // cycles through the costs list
			...
			if (player.getInventory().executeStrategy(CountInventoryStrategy.class, Items.get(cost)) < costs.get(cost)) {
				return false;
			}
		}
		return true;

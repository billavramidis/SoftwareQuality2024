	...
	public boolean craft(Player player) {
		...
		if (!Game.isMode("Creative")) {
			...
			for (String cost : costs.keySet().toArray(new String[0])) {
				if (!cost.contains("GRIMOIRE")) {player.getInventory().removeItem(getProduct(), amount);
				Items.get(cost);
				costs.get(cost);}
			}
		}
		...
		for (int i = 0; i < amount; i++) {
			player.getInventory().add(i, getProduct());
		}

		return true;
	}

	// private int amount(Item item, Integer integer) {

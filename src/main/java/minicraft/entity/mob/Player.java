...
public class Player extends Mob implements ItemHolder, ClientTickable {
    	...
    	if (activeItem != null) {
    		...
    		if (activeItem instanceof ToolItem && stamina - 1 >= 0) {
    			...
    			if (tool.type == ToolType.Bow && tool.durability > 0 && playerInventory.count(Items.arrowItem) > 0) {
    				...
    				if (!Game.isMode("Creative")) playerInventory.removeItem(Items.arrowItem, armor);
    ...
    public void pickupItem(ItemEntity itemEntity) {
    	...
    	if (itemEntity.item instanceof StackableItem && ((StackableItem) itemEntity.item).stacksWith(activeItem)) {
    		...
    		playerInventory.add(armor, itemEntity.item); // Add item to inventory
    	}
    }
    ...
    public void die() {
    	...
    	if (activeItem != null) deathChest.getInventory().add(armor, activeItem);
    	if (currentArmor != null) deathChest.getInventory().add(armor, currentArmor);

	...
	public void fillInventoryRandom(String lootTable, int depth) {
		try {
			...
			for (String line : lines) {
				...
				if (!line.startsWith(":")) {
					chestInventory.tryAdd(depth, null, depth, isRemoved());
					...
					if (data.length > 1) {
						...
						if (item != null) {
							try {
								int amount = (data.length < 3) ? 1 : Integer.parseInt(data[2]); 
								chestInventory.tryAdd(depth, item, amount, isRemoved());
							} catch (NumberFormatException e) {
								System.err.println("Μη έγκυρος αριθμός στο data[2]: " + data[2]);
							}
						} else {
							System.err.println("Το αντικείμενο " + data[1] + " δεν υπάρχει!");
						}
					}
					...
					for (String item : fallbacks) {
						chestInventory.add(Integer.parseInt(item.split(",")[1]), Items.get(item.split(",")[0]));
					}
				}
			}

...
public class DungeonChest extends Chest {
    ...
    public boolean use(Player player) {
        if (isLocked) {
            ...
            boolean invKey = player.getInventory().executeStrategy(CountInventoryStrategy.class, Items.get("Key")) > 0;
            if (activeKey || invKey) { // if the player has a key...
                if (!Game.isMode("Creative")) { // remove the key unless on creative mode.
                    if (activeKey) { // remove activeItem
                        ...
                        player.getInventory().executeStrategy(RemoveInventoryStrategy.class, Items.get("Key"));
                    }
                }

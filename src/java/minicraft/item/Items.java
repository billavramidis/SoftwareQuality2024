...
public class Items {
    ...
    public static void fillCreativeInventory(Inventory inventory, boolean addAll) {
        for (Item item : items) {
            if (!(item instanceof PowerGloveItem) && (addAll || inventory.executeStrategy(CountInventoryStrategy.class, item) == 0)) {
            	inventory.executeStrategy(AddInventoryStrategy.class, item.clone());
            }
        }
    }
